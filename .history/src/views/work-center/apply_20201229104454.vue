<template>
  <div>
          ...
          <el-form-item label="上传附件" prop="field118">
            ...
            >
              <el-button size="small" type="primary" icon="el-icon-upload"
                >点击上传</el-button
              >
             <span style="margin-left:8px;font-size:12px;color: #606266;">上传文件格式：jpg/png/jpeg/xls/xlsx/doc/docx/pdf/ppt/pptx/rar/zip（建议不超过20M）</span>
            </el-upload>
          </el-form-item>
        </el-col>
      </el-form>
    ...
    <template v-if="currentFormKey === 'WlbgFormKey4' || evaluationData.formKey === 'WlbgFormKey4'">
      ...
      <el-row :gutter="10">
        <el-form ref="evalutionForm" :model="evaluationData" :rules="evaluationRules" size="small" label-width="120px" :disabled="currentFormKey!=='WlbgFormKey4'">
          <el-col :span="8">
      ...
      <el-form ref="approveForm" :model="approveDatas" :rules="approveRules" label-width="120px">
        <el-col :span="11">
          <el-form-item class="is-required" label="请选择：" prop="approveFlag">
            <el-radio-group v-model="approveDatas.approveFlag"  @change="changeApprove">
              <el-radio :label="0">同意</el-radio>
              <el-radio :label="1" v-if="!(currentFormKey==='WlbgFormKey5' && hisFormDatas.filter(item=>item.formKey==='WlbgFormKey3'||item.formKey==='WlbgFormKey4').length > 0)&&currentFormKey!=='WlbgFormKey3'&&currentFormKey!=='WlbgFormKey1'&&currentFormKey!=='WlbgFormKey2'">不同意</el-radio>
            </el-radio-group>
          </el-form-item>
        </el-col>
        <el-col :span="22">
        ...
        <el-col :span="10" v-if="currentFormKey === 'WlbgFormKey2'">
          <el-form-item label="请选择实施部门领导" class="is-required" size="small">
            <el-select
              style="width: 100%"
              filterable
        ...
        <el-col :span="24" class="btn-group">
          <el-form-item size="large">
            <el-button type="primary" @click="validate" :loading="isClickFlag">提交</el-button>
  ...
  methods: {
    ...
    validate() {
      if(this.currentFormKey==='WlbgFormKey1') {
        ...
        validateForms([this.$refs["approveForm"]]).then(()=>{
         this.submitForm()
        }).catch(err => {console.log(err)})
      } else if(this.currentFormKey==='WlbgFormKey3') {
        validateForms([this.$refs['shishiForm'],this.$refs["approveForm"]]).then(()=>{
         this.submitForm()
        }).catch(err => {console.log(err)})
      }else if(this.currentFormKey==='WlbgFormKey4') {
         validateForms([this.$refs["evalutionForm"],this.$refs["approveForm"]]).then(()=>{
           this.submitForm()
        }).catch(err => {console.log(err)})
      }else {
        validateForms([this.$refs["approveForm"]]).then(()=>{
