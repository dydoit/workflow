  ...
  <div>
    ...
    width="25%"
     :visible.sync="dialogVisible">
      <el-select style="width:100%" v-model="leaderOa" placeholder="请选择" @change="selectLeader">
        <el-option
...
export default {
  ...
  methods: {
    ...
    selectLeader(val){
      if(val) {
        this.leaderName = this.leaderObj[val].name
      }
    },
    async sureSubmit(){
      let params = {
  ...
              fileComment:item.title
    ...
    handleUpload(res, file, fileList) {
      if (res.code === 0) {
        let { url, fileName, userName, uploadTime,id } = res;
