  ...
  <div>
    <el-row :gutter="0">
      <el-col :span="22" class="his-wrap">
         <el-button type="primary" size="small" icon="el-icon-search" @click="hisDrawer=true">查看审批历史</el-button>
      </el-col>
    </el-row>
     <!-- 申请的表单 -->
...
export default {
  ...
  created() {
    ...
    this.taskId = taskId;
    this.getLeaderList()
    processInstanceId && taskId && this.getFormTableByPid(processInstanceId, taskId);
    processInstanceId && taskId &&
      this.getAttachmentByProcessInstanceId(processInstanceId,taskId);
    processInstanceId &&
      this.getCommentsByProcessInstanceId(processInstanceId);
  ...
  methods: {
    ...
    async getCommentsByProcessInstanceId(processInstanceId) {
      let res = await this.$http.getCommentsByProcessInstanceId({processInstanceId})
      if(res.code === 0) {
    ...
    handleDelStart({fileId}) { // 删除附件(
      this.$confirm('此操作将永久删除该文件, 是否继续?', '提示', {
        ...
        if(res.code===0) {
          this.startAttachmentList = this.startAttachmentList.filter(item => item.fileId !== fileId)
        }
      }).catch(() => {
          this.$message({
            type: 'info',
    ...
    async submitForm() {
      ...
      const formData =
        this.currentFormKey === "WlbgFormKey2"
          ...
          : this.currentFormKey === "WlbgFormKey1"?{...this.formData}:null;
      const fileDatas =
        this.currentFormKey === "WlbgFormKey3"
          ...
          : this.currentFormKey === "WlbgFormKey1"?this.startAttachmentList.map((item) => {
              return {
                ...item,
                fileType: this.currentFormKey,
              };
