<template>
  <div>
    ...
    <template
      v-if="
        currentFormKey === 'WlbgFormKey3' ||
        shishiData.formKey === 'WlbgFormKey3'
      "
    >
    ...
    <template
      v-if="
        currentFormKey === 'WlbgFormKey4' ||
        evaluationData.formKey === 'WlbgFormKey4'
      "
    >
      ...
      <el-row :gutter="10">
        ...
        >
          <el-col :span="8">
            <el-form-item prop="receiptQualityScore">
              <span slot="label" class="auto-label"
                >回单质量
                <el-popover
                  placement="top"
          ...
          <el-col :span="8">
            <el-form-item prop="receiptTimelyScore">
              <span slot="label" class="auto-label"
                >回单及时性
                <el-popover
                  placement="top"
    ...
    <el-row :gutter="15">
      ...
      >
        <el-col :span="11">
          <el-form-item class="is-required" label="请选择：" prop="approveFlag">
            <el-radio-group
            ...
            >
              ...
              <el-radio
                ...
                v-if="
                  !(
                    currentFormKey === 'WlbgFormKey5' &&
                    hisFormDatas.filter(
                      (item) =>
                        item.formKey === 'WlbgFormKey3' ||
                        item.formKey === 'WlbgFormKey4'
                    ).length > 0
                  ) &&
                  currentFormKey !== 'WlbgFormKey3' &&
                  currentFormKey !== 'WlbgFormKey1' &&
                  currentFormKey !== 'WlbgFormKey2'
                "
                >不同意</el-radio
              >
            </el-radio-group>
        ...
        <el-col :span="22">
          <el-form-item label="请填写意见：" prop="approveMsg">
            <el-input
              type="textarea"
              :rows="2"
              placeholder="请输入内容"
        ...
        <el-col :span="10" v-if="currentFormKey === 'WlbgFormKey1'">
          <el-form-item label="请选择审批人" size="small" class="is-required">
            <el-select
              style="width: 100%"
              filterable
        ...
        <el-col :span="24" class="btn-group">
          <el-form-item size="large">
            <el-button type="primary" @click="validate" :loading="isClickFlag"
              >提交</el-button
            >
...
const pathHead = "http://10.117.173.170:2000/uniflowFileSystem";;
...
export default {
  ...
  watch: {
    ...
    "formData.applySiteCode"(newVal) {
      this.getLeaderList(newVal);
    },
  },
  created() {
  ...
  methods: {
    showDrawer() {
      if (this.currentFormKey !== "WlbgFormKey1") return;
      this.drawer = true;
    },
    changeApprove(val) {
      if (val === 0) {
        this.approveDatas.approveMsg = "同意";
      } else {
        this.approveDatas.approveMsg = "不同意";
      }
    },
    filter1() {
    ...
    async getLeaderList(code) {
      let res = await this.$http.queryLeaderList({
        limit: 1000,
        page: 1,
        siteCode: code, // 应该是当前表单获取的部门代码
      });
      if (res.code === 0) {
        this.leaderOptions =
          res.page &&
    ...
    async getAttachmentByProcessInstanceId(processInstanceId, taskId) {
      ...
      if (res.code === 0) {
        ...
        console.log(res.msg ? res.msg : "接口错误");
      }
    },
    async getCurrentFromJson(processInstanceId, taskId) {
      ...
      if (res.code === 0) {
        ...
        if (this.currentFormKey === "WlbgFormKey3") {
          ...
          this.shishiData.handleTimely =
            new Date(this.formData.completeDate).getTime() -
              new Date(this.shishiData.completionTime).getTime() >=
            0
              ? "正常"
              : "超时";
        }
      } else {
        alert(res.msg ? res.msg : "接口错误");
      }
    },
    async getHisFromDatas(processInstanceId) {
      ...
      if (res.code === 0) {
        ...
        let shishiDataObj = res.formDatas.filter(
          (item) => item.formKey === "WlbgFormKey3"
        )[0];
        let evaluationDataObj = res.formDatas.filter(
          (item) => item.formKey === "WlbgFormKey4"
        )[0];
        if (shishiDataObj) {
          this.shishiData = {
    ...
    handleSee({ fileUrl }) {
      //预览
      const seePath = "http://10.117.173.170:2000/filepreview/onlinePreview?url=";;
    ...
    handleDelEvaluation({ fileId, deleteFuctionType }) {
      // 评价阶段删除附件(
      this.$confirm("此操作将永久删除该文件, 是否继续?", "提示", {
        ...
        .then(async () => {
          let res = {};
          if (deleteFuctionType === "0") {
            // 删新上传的附件
            res = await this.$http.delAttach({
              id: fileId,
            });
          } else if (deleteFuctionType === "1") {
            //删和工单绑定的附件
            res = await this.$http.delProAttach({
              pId: this.processInstanceId,
              taskId: this.taskId,
              id: fileId,
            });
          } else {
            console.log("deleteFuctionType错误");
          }
          if (res.code === 0) {
            this.evaluationAttachmentList = this.evaluationAttachmentList.filter(
    ...
    handleDelStart({ fileId, deleteFuctionType }) {
      // 删除开始表单附件(
      this.$confirm("此操作将永久删除该文件, 是否继续?", "提示", {
        confirmButtonText: "确定",
        cancelButtonText: "取消",
    ...
    handleDelShishi({ fileId, deleteFuctionType }) {
      // 删除实施阶段附件(
      this.$confirm("此操作将永久删除该文件, 是否继续?", "提示", {
        confirmButtonText: "确定",
        cancelButtonText: "取消",
        ...
        .then(async () => {
          ...
          if (res.code === 0) {
            this.shishiAttachmentList = this.shishiAttachmentList.filter(
              (item) => item.fileId !== fileId
            );
          }
        })
    ...
    validate() {
      if (this.currentFormKey === "WlbgFormKey1") {
        validateForms([this.$refs["elForm"], this.$refs["approveForm"]])
          .then(() => {
            this.submitForm();
          })
          .catch((err) => {
      ...
      } else if (this.currentFormKey === "WlbgFormKey2") {
        validateForms([this.$refs["approveForm"]])
          .then(() => {
            this.submitForm();
          })
          .catch((err) => {
      ...
      } else if (this.currentFormKey === "WlbgFormKey3") {
        validateForms([this.$refs["shishiForm"], this.$refs["approveForm"]])
          .then(() => {
            this.submitForm();
          })
          .catch((err) => {
      ...
      } else if (this.currentFormKey === "WlbgFormKey4") {
        validateForms([this.$refs["evalutionForm"], this.$refs["approveForm"]])
          .then(() => {
            this.submitForm();
          })
          .catch((err) => {
    ...
    async submitForm() {
      ...
      if (this.currentFormKey === "WlbgFormKey2") {
        // 实施部门领导oa必选
        if (!this.shishibumenOa) {
          this.$message.error("请选择实施部门领导");
          return;
        }
      }
      if (this.isClickFlag) {
