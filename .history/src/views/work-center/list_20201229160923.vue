<template>
  <div class="work-list">
    ...
    <el-table
      ...
      size="small"
    >
      <el-table-column prop="formName" label="工单标题" width="260">
      </el-table-column>
      <el-table-column prop="approver" label="上级审批人" width="80">
      </el-table-column>
      <el-table-column prop="processDefId" label="工单类别" width="130">
      ...
      <el-table-column label="操作">
        <template slot-scope="scope">
          ...
          >
          <el-button
            v-if="scope.row.enableRevokeFlag === '0'"
            style="color: red"
            type="text"
            size="mini"
          ...
          >
          <el-button
            v-if="scope.row.enableStopFlag === '0'"
            style="color: red"
            type="text"
            size="mini"
    ...
    <el-dialog :title="title" width="35%" :visible.sync="dialogVisible">
      <el-form
        ref="elForm"
        :model="formData"
        :rules="formRules"
...
export default {
  ...
  methods: {
    ...
    async handleClick({ processInstanceId }) {
      // 展示详情
    ...
    async stopProcess() {
      // 终止
      let res = await this.$http.stopProcess({
        processInstanceId: this.processInstanceId,
        userCode: this.user.oa,
        message: this.formData.reason,
      });
