  ...
  methods: {
    ...
    async getHisFromDatas(processInstanceId) {
      ...
      if (res.code === 0) {
        ...
        let shishiData = res.formDatas.filter(item => item.formKey === 'WlbgFormKey3')[0]
        console.log(shishiData)
        let evaluationData = res.formDatas.filter(item => item.formKey === 'WlbgFormKey4')
      }
      console.log(res);
    },
    handleSee({ fileUrl }) {
