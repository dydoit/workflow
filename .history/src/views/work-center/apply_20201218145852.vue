      ...
      <el-form ref="form" :model="approveDatas" label-width="120px">
        ...
        <el-col :span="10" v-if="currentFormKey==='WlbgFormKey2'">
          <el-form-item label="请选择审批人" class="is-required" size="small">
            <el-select style="width:100%" v-model="shishibumenOa" placeholder="请选择" @change="selectShishiLeader">
          <el-option
...
export default {
  ...
  methods: {
    ...
    async getshishiLeaderList(){ // 获取实施部门领导list
      ...
      if(res.code === 0) {
        this.shishiLeaderOptions = res.page&&res.page.list.map(item => {
          return {
            value: item.oa,
            label: item.name
          }
        })
        this.shishiLeaderObj = res.page&&res.page.list.reduce((last,cur)=>{
          return {
            ...last,
            [cur.oa]:cur
          }
